...
# 4. 참진드기조사 어린이숲체험장 레이어 (💡 년도별 조사 지점 완전 자동 유연 변환 파트)
    ...
    with forest_tabs[0]:
        st.markdown(f"#### [{selected_year} {selected_month}] 데이터 정밀 분석")
        try:
    ...
    with forest_tabs[1]:
        st.markdown("#### 🌍 전체 년도 통합 누적 거점 및 채집량 분석")
        
        if not df_forest.empty:
            df_all = df_forest.copy()
            
            # 연도 문자열 포맷팅
            df_all["조사년도_str"] = df_all["조사년도"].astype(str).str.strip()
            df_all["조사년도_str"] = df_all["조사년도_str"].apply(lambda x: x if "년" in x else x + "년")
            
            df_all = df_all.dropna(subset=['채집지역2'])
            
            def safe_int_all(x):
                try: return int(float(x))
                except: return 1
            df_all['지점번호'] = df_all['지점번호'].apply(safe_int_all)
            
            def calc_gu_spot_all(x):
                cls = str(x['분류']).strip().lower()
                num = x['지점번호']
                is_in = cls in ["in", "탐방로"]
                if num in [1, 2, 3, 4, 5, 6]:
            ...
            df_all['gu분지점'] = df_all.apply(calc_gu_spot_all, axis=1)
            
            # 년도 및 지역별 정확한 좌표 통합 연동
            def get_int_coords(year_val, region_val):
                y_str = str(year_val)
                r_str = str(region_val).strip()
                h_c = {
                    "2025": {"홍천": [37.7336, 127.8547], "정선": [37.4922, 128.9814]},
                    "2024": {"춘천": [37.9799, 127.7718], "인제": [38.0620, 128.1560]},
                    "2023": {"속초": [38.2043, 128.5919], "양양": [38.0754, 128.6195], "인제": [38.0620, 128.1560]}
                }
                default_c = {"남산": [37.683361, 127.893111], "삼마치": [37.643444, 127.910306]}
                
                if "2025" in y_str: target_dict = h_c["2025"]
                elif "2024" in y_str: target_dict = h_c["2024"]
                elif "2023" in y_str: target_dict = h_c["2023"]
                else: target_dict = default_c
                
                if r_str in target_dict: return target_dict[r_str]
                elif r_str in default_c: return default_c[r_str]
                return [37.665, 127.900]
                
            coords = df_all.apply(lambda row: get_int_coords(row['조사년도_str'], row['채집지역2']), axis=1)
            df_all['위도'] = [c[0] for c in coords]
            df_all['경도'] = [c[1] for c in coords]
            
            col_m_all, col_g_all = st.columns([5, 5])
            with col_m_all:
                st.markdown("##### 🗺️ 전체 년도 관측 지점 누적 매핑 (연도별 컬러 마커)")
                m_all = folium.Map(location=[37.8, 128.2], zoom_start=8)
                
                folium_colors = {"2026년": "red", "2025년": "blue", "2024년": "green", "2023년": "purple", "2021년": "orange", "2020년": "darkgreen"}
                
                map_grouped = df_all.groupby(["조사년도_str", "채집지역2", "위도", "경도"])["개체수"].sum().reset_index()
                
                for _, r in map_grouped.iterrows():
                    y = r["조사년도_str"]
                    reg = r["채집지역2"]
                    lat, lng = r["위도"], r["경도"]
                    tot = r["개체수"]
                    
                    # 마커 겹침 방지 보정 (인제 등 동일 좌표 중첩용)
                    if reg == "인제":
                        if "2024" in y: lat, lng = lat + 0.015, lng - 0.015
                        elif "2023" in y: lat, lng = lat - 0.015, lng + 0.015

                    marker_color = folium_colors.get(y, "gray")
                    popup_text = f"<b>[{y}] {reg}</b><br>총 개체수: {int(tot)}개체"
                    folium.Marker([lat, lng], tooltip=f"[{y}] {reg}", popup=folium.Popup(popup_text, max_width=300), icon=folium.Icon(color=marker_color, icon='flag')).add_to(m_all)
                    
                st_folium(m_all, key="map_forest_all_years", width="100%", height=430)
                
            with col_g_all:
                st.markdown("##### 📊 관리/비관리지점 년도별 스택 통합 채집량")
                fig, ax = plt.subplots(figsize=(6, 5))
                
                chart_df_all = df_all.pivot_table(index="gu분지점", columns="조사년도_str", values="개체수", aggfunc="sum").fillna(0)
                
                # 요청하신 6개 지점으로 완벽 통합 
                target_indices = ["관리지점 1", "관리지점 2", "관리지점 3", "비관리지점 1", "비관리지점 2", "비관리지점 3"]
                chart_df_all = chart_df_all.reindex(target_indices).fillna(0)
                
                sorted_cols = sorted(chart_df_all.columns, reverse=True)
                chart_df_all = chart_df_all[sorted_cols]
                
                plt_colors = {"2026년": "#d62728", "2025년": "#1f77b4", "2024년": "#2ca02c", "2023년": "#9467bd", "2021년": "#ff7f0e", "2020년": "#103b10"}
                plot_colors = [plt_colors.get(c, "#7f7f7f") for c in chart_df_all.columns]
                
                chart_df_all.plot(kind='bar', stacked=True, ax=ax, color=plot_colors, edgecolor='black', width=0.6)
                plt.xticks(rotation=0)
                plt.legend(title="조사년도", fontsize=8, title_fontsize=9)
                plt.tight_layout()
                st.pyplot(fig)
                plt.close()
                
            st.dataframe(df_all.groupby(["조사년도_str", "채집지역2", "gu분지점"])["개체수"].sum().reset_index().rename(columns={"조사년도_str": "조사년도"}), hide_index=True, use_container_width=True)
        else:
            st.info("💡 통합 분석할 참진드기 조사 데이터가 존재하지 않습니다.")
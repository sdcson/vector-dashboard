...
def get_cli_tick_data():
    ...
    # 💡 [강력 패치] 데이터 로드 시점에 '기타'라는 텍스트가 있으면 무조건 'Larva'로 덮어쓰기
            ...
            with c2:
                ...
                if not m_data.empty and "종" in m_data.columns and loc_col in m_data.columns:
                    ...
                    if not all_spot_clean.empty:
                        # 💡 [핵심] '기타'를 무조건 'Larva'로 합산하여 범례에 '기타'가 절대 뜨지 않도록 처리
                        ...
                        # 💡 [핵심] 0마리 잡힌 지점도 무조건 리스트에 남겨서 X축에 표출되게 만듦
...
# =================================================================================
    ...
    if not m_forest.empty:
        # 💡 [핵심] '기타'를 'Larva'로 통일 (Hard tick, Haemaphysalis... 치환 전에)

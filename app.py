...
# -----------------------------------------------------------------
# [💡 기상청 오픈 API 실시간 연동 모듈 - 로딩 지연 완전 해결]
# -----------------------------------------------------------------
@st.cache_data(ttl=3600)
def get_kma_weather(year_str, month_str, week_str, loc_name):
    """특정 주차/월간 단일 데이터를 빠르게 호출하는 엔진"""
    ...
    try:
        res = requests.get(url, timeout=10) # 💡 타임아웃 10초로 연장
...
def get_kma_weather_bulk(year_str, loc_name):
    """💡 [신규] 상관분석 탭 전용: 3월~10월 데이터를 단 한 번의 호출로 묶어 처리 (로딩 속도 8배 향상)"""
    y = str(year_str).replace("년", "").strip()
...
# =================================================================================
    ...
    with col_c4:
        climate_factors = st.multiselect("비교할 기후 인자", ["평균기온(°C)", "누적강수량(mm)", "평균습도(%)"], default=["평균기온(°C)", "누적강수
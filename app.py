...
def safe_parse_month_series(series, default_val):
    def _parse(val):
        ...
        try:
            val_str = str(val).strip()
            if "월" in val_str: val_str = val_str.replace("월", "").strip()
...
def parse_vectornet_dataframe(df, default_year, default_month):
    ...
    if "월.1" in df.columns and "월" in df.columns:
        ...
        if "연도" in df.columns: df["조사년도"] = safe_parse_year_series(df["연도"], default_year)
        elif "년도" in df.columns: df["조사년도"] = safe_parse_year_series(df["년도"], default_year)
        else: df["조사년도"] = default_year
            
        if "월" in df.columns: df["조사월"] = safe_parse_month_series(df["월"], default_month)
        else: df["조사월"] = default_month
    return df
    ...
    if not df_je.empty:
        ...
        if not f_je.empty:
            ...
            with je_sub_tabs[0]:
                ...
                with c2:
                    # 기존 겹치던 날씨 선 그래프 제거 후 본연의 막대그래프 복구
                    ...
                    if not df_ct.empty:
                        for _, row in df_ct.iterrows():
                            ...
                            for s in all_je_short_spots:
                                if s in loc_str:
                                    spot_dict[s] += row.get(val_col_je, 0)
...
# =================================================================================
# 5. 💡 [신규] 기상 요인 및 매개체 발생 상관분석 레이어 (일본뇌염)
# =================================================================================
elif selected_tab == "☁️ 기상 요인 상관분석":
    st.header(f"☁️ 기후 요인 및 매개체 발생 상관분석 ({selected_year} 시즌)")
    ...
    with col_c3:
        climate_factors = st.multiselect("비교할 기후 인자 선택", ["평균기온(°C)", "누적강수량(mm)", "평균습도(%)"], default=["평균기온(°C)", "누적강수량(mm)"])
    ...
    if not df_je.empty:
        ...
        # 년도 및 지점 필터링
        f_je = df_je[df_je["조사년도"] == selected_year].copy()
        ...
        # 기상청 API 월간 데이터 호출
        with st.spinner("기상청 API 실시간 연동 중..."):
            weather_data = []
            for m in months:
                w = get_kma_weather(selected_year, m, "전체", selected_spot)
                weather_data.append(w)
                
            plot_df["평균기온(°C)"] = [w["temp"] for w in weather_data]
            plot_df["누적강수량(mm)"] = [w["precip"] for w in weather_data]
            plot_df["평균습도(%)"] = [w["humid"] for w in weather_data]
        
        # 이중축 연동 차트 그리기
        st.markdown(f"##### 📊 {selected_spot} 작은빨간집모기 계절적 변화 추이 (3월~10월)")
        ...
        st.info("💡 해당 연도의 일본뇌염 데이터가 존재하지 않아 기후 분석을 생성할 수 없습니다.")